...
def usage():
    help_text = """read.py usage
    There are two main methods:
    - `organize_mast_files(files)`
      This parses the filenames generated by the pipeline and organizes them
      into a dataframe for sorting. It can optionally add a column for a header
      keyword, too.
    - `organize_files_by_header(files, ext=0)`
      This pulls out all the keywords for a given header and makes a dataframe
      with them. 
    """
    print(help_text)
...
def organize_mast_files(files: list, extra_keys: dict[str, int] = {}):
    ...
    extra_keys: a list of extra keywords you want to add to the dataframe. Format is key: header_int
      some useful examples: {'SUBARRAY': 0, 'FILTER': 0, 'EXP_TYPE': 0, 'BKGDTARG': 0}
    ...
    for i in range(len(files))[::-1]:
        if Path(files[i]).name[7] == '-': # file is a stage 3 file, drop it
            ff = files.pop(i)
            # use ff later for stage 3 stuff
    ...
    for k in fits_keys:
        data_organizer[k.lower()] = None
    # invert the extra key dictionary:
    extra_keys = {v: [k for k in extra_keys.keys() if extra_keys[k] == v]
                  for v in extra_keys.values()}
    # row-wise function that only reads the fits file once
    def getkeys(row, ek):
        path = Path(row['path']) / row['filename']
        vals = {}
        for hdr_id, keys in ek.items():
            hdr = fits.getheader(path, hdr_id)
            for k in keys:
                vals[k] = hdr[k]
        # assign the fits keys to the dataframe
